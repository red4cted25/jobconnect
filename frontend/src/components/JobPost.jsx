...
const JobPost = ({
    _id, // use the MongoDB id instead of jobId
    title,
    company,
    location,
    remote,
    ...
    return (
        ...
        <div className="flex flex-col md:flex-row md:items-start gap-4">
            ...
            <Link to={`/jobs/apply/${_id}`} className="w-full bg-brand-primary text-white py-2 px-4 rounded-md uppercase text-center text-sm font-medium hover:bg-brand-primary-light transition-colors">
                Apply Now
            </Link>
            <Link to={`/jobs/post/${_id}`} className="w-full bg-white border border-brand-primary text-brand-primary py-2 px-4 rounded-md uppercase text-center text-sm font-medium hover:bg-brand-primary-light hover:text-white transition-colors">
                Learn More
            </Link>
            </div>
        </div>
